...
for directory in "$runtime_root/functions" "$runtime_root/completions"
    ...
    if string match --quiet '*/functions' -- "$directory"
        contains -- "$directory" $fish_function_path; or set --prepend fish_function_path "$directory"
    else
        contains -- "$directory" $fish_complete_path; or set --prepend fish_complete_path "$directory"
    end
end
